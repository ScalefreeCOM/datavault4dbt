...
## ATTENTION: Upgrading to v2.0.0

v2.0.0 unifies and corrects the hash standardization logic across all adapters. Most notably, the case-sensitivity handling was corrected: with the default settings, **hashkeys are now `UPPER`-normalized** (previously they were not) and **hashdiffs are no longer `UPPER`-normalized** (previously they were). Several adapters carry additional output-affecting changes (Oracle/Redshift `concat_string` alignment, BigQuery MD5 conversion fix, Fabric and Exasol standardization changes).

As a result, hash values generated by v2.0.0 differ from those generated by earlier versions. When upgrading an existing Data Vault from v1 to v2, you must either **fully refresh the RDV** (if the source history is available) or **rehash** all existing entities using the macros described below.

## ATTENTION: Delimiter collision hardening

The hashing logic concatenates the input columns as `"value"||"value"||^^|...`, using the `quote_character` (`"`) to wrap each value, the `concat_string` (`||`) to join them, and the `null_placeholder_string` (`^^`) to mark NULLs. To guarantee that these structural delimiters can never be confused with real data, any occurrence of them *inside* an input value is now substituted with a dedicated replacement token before concatenation (`concat_string_replacement`, `quote_character_replacement` and `null_placeholder_string_replacement` — see [Global Variables](../29_global-variables/29_global-variables.md)).
