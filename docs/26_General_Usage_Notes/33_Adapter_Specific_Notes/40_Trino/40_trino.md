...
- **String Aggregation:** Trino does not support `STRING_AGG`. The package uses `LISTAGG(expression, ',') WITHIN GROUP (ORDER BY multi_active_key)` as the native equivalent for multi-active satellite hashdiff calculations.
