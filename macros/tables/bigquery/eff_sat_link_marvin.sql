...
--Equals the Eff_Sat_hub macro provided in the wiki

{%- macro eff_sat_link_marvin(link, src_pk, src_dk, src_ldts, src_rsrc, source_model) -%}

    {{ adapter.dispatch('eff_sat_link_marvin', 'dbtvault_scalefree')(link=link, 
                                                  src_pk=src_pk,
                                                  src_dk=src_dk,
                                                  src_ldts=src_ldts,
                                                  src_rsrc=src_rsrc,
...
{%- macro default__eff_sat_link_marvin(link, src_pk, src_dk, src_ldts, src_rsrc, source_model) -%}

{{- dbtvault_scalefree.check_required_parameters(link=link, 
                                                 src_pk=src_pk,
                                                 src_dk=src_dk,
                                                 src_ldts=src_ldts, 
                                                 src_rsrc=src_rsrc,
...
{%- set source_cols = dbtvault_scalefree.expand_column_list(columns=[src_pk, src_dk, src_ldts, src_rsrc]) -%}
...
esat_latest AS (
    SELECT *
    FROM  {{ this }}
    QUALIFY ROW_NUMBER() OVER (PARTITION BY {{ src_dk }} ORDER BY {{ src_ldts }} DESC) = 1
    ),
{%- endif %}
