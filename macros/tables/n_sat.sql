{%- macro n_sat(src_pk, src_payload, src_eff, src_ldts, src_source, source_model) -%}
    ...
    {{- adapter.dispatch('n_sat', 'dbtvault')(src_pk=src_pk, src_payload=src_payload, 
...
{{- dbtvault.check_required_parameters(src_pk=src_pk, src_payload=src_payload,
                                       src_ldts=src_ldts, src_source=src_source,
                                       source_model=source_model) -}}

{%- set source_cols = dbtvault.expand_column_list(columns=[src_pk, src_ldts, src_source, src_payload, src_eff]) -%}
...
records_to_insert AS (
    ...
    {%- if dbtvault.is_any_incremental() %}
        LEFT JOIN latest_records
            ...
            WHERE {{ dbtvault.prefix([src_pk], 'latest_records', alias_target='target') }} IS NULL
    {%- endif %}
)
