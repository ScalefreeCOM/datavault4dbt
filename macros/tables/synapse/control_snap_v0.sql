...
enriched_timestamps AS (
    ...
    SELECT 
        ...
        {%- else %}
        CAST(DATEADD(week, DATEDIFF(week, 0, {{ sdts_alias }}), 0) AS DATE) AS beginning_of_week,
        CAST(DATEADD(day, 6, DATEADD(week, DATEDIFF(week, 0, {{ sdts_alias }}), 0)) AS DATE) AS end_of_week,
        {%- endif %}
        CAST(DATEADD(month, DATEDIFF(month, 0, {{ sdts_alias }}), 0) AS DATE) AS beginning_of_month,
        CAST(EOMONTH({{ sdts_alias }}) AS DATE) AS end_of_month,
        CAST(DATEADD(quarter, DATEDIFF(quarter, 0, {{ sdts_alias }}), 0) AS DATE) AS beginning_of_quarter,
        CAST(DATEADD(day, -1, DATEADD(quarter, 1, DATEADD(quarter, DATEDIFF(quarter, 0, {{ sdts_alias }}), 0))) AS DATE) AS end_of_quarter,
        CAST(DATEADD(year, DATEDIFF(year, 0, {{ sdts_alias }}), 0) AS DATE) AS beginning_of_year,
        CAST(DATEADD(day, -1, DATEADD(year, 1, DATEADD(year, DATEDIFF(year, 0, {{ sdts_alias }}), 0))) AS DATE) AS end_of_year,
        NULL AS comment
