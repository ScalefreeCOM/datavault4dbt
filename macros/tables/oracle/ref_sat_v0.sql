{%- macro oracle__ref_sat_v0(parent_ref_keys, src_hashdiff, src_payload, src_ldts, src_rsrc, source_model) -%}
...
latest_entries_in_sat AS (
    ...
    FROM (
            SELECT
                ...
                {{ref_key}},
                {% endfor %}
                ROW_NUMBER() OVER(PARTITION BY {%- for ref_key in parent_ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }} DESC) AS latest,
                {{ ns.hdiff_alias }}
            FROM {{ this }}
          )
    WHERE latest = 1
...
deduplicated_numbered_source AS (
    ...
    FROM (
            SELECT
            CASE
                WHEN {{ ns.hdiff_alias }} = LAG({{ ns.hdiff_alias }}) OVER(PARTITION BY {%- for ref_key in parent_ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }}) THEN 0
                ELSE 1
            END AS latest,
            {% for ref_key in parent_ref_keys %}
            {{ref_key}},
