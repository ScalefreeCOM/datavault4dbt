...
(
    ...
    QUALIFY CASE WHEN {{ datavault4dbt.prefix([link_hashkey], 'source_model') }} = LAG({{ datavault4dbt.prefix([link_hashkey], 'source_model') }}) OVER (PARTITION BY {{ datavault4dbt.prefix([driving_key], 'source_model') }} ORDER BY {{ datavault4dbt.prefix([src_ldts], 'source_model') }}) THEN FALSE
                 ELSE TRUE
            END
),
