...
eff_ranges AS (
    ...
    SELECT
        ...
        COALESCE(LAG(ADD_SECONDS({{ src_ldts }}, -0.001)) OVER (PARTITION BY {{ link_hashkey }} ORDER BY {{ src_ldts }} DESC), {{ datavault4dbt.string_to_timestamp( timestamp_format , end_of_all_times) }}) as {{ eff_to_alias }}
    FROM source_data
