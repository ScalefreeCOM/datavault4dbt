{%- macro databend__control_snap_v0(start_date, daily_snapshot_time, sdts_alias) -%}
...
initial_timestamps AS (
    ...
    SELECT
        DATE_ADD(DAY, SEQ4(),
        DATE_ADD(SECOND, EXTRACT(SECOND FROM TO_TIMESTAMP('{{ daily_snapshot_time }}')),
            DATE_ADD(MINUTE, EXTRACT(MINUTE FROM TO_TIMESTAMP('{{ daily_snapshot_time }}')),
                DATE_ADD(HOUR, EXTRACT(HOUR FROM TO_TIMESTAMP('{{ daily_snapshot_time }}')), TO_DATE('{{ start_date }}', 'YYYY-MM-DD')))
                ))::TIMESTAMP AS sdts
    FROM 
