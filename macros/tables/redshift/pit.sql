...
SELECT
    ...
    {% for satellite in sat_names %}
        ...
        COALESCE(MAX({{ satellite }}.{{ hashkey }}), CAST({{ datavault4dbt.as_constant(column_str=unknown_key) }} as {{ hash_dtype }})) AS hk_{{ satellite }},

        COALESCE(MAX({{ satellite }}.{{ ldts }}), {{ datavault4dbt.string_to_timestamp(timestamp_format, beginning_of_all_times) }}) AS {{ ldts }}_{{ satellite }}
...
WHERE snap.{{ sdts }} NOT IN (SELECT DISTINCT {{ sdts }} FROM {{ this }})
