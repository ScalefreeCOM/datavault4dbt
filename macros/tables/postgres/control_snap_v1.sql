{%- macro postgres__control_snap_v1(control_snap_v0, log_logic, sdts_alias) -%}
        ...
        {%- else %}
            {% for logic in ns.log_logic_list -%}
                ...
                CASE
                    ...
                    (c.{{ sdts_alias }} BETWEEN CURRENT_DATE - INTERVAL '{{ weekly_duration }} {{ weekly_unit }}' AND CURRENT_DATE AND (c.is_beginning_of_week = TRUE))
                        {%- endif -%}
                    {% endif -%}
                    ...
                    (c.{{ sdts_alias }} BETWEEN CURRENT_DATE - INTERVAL '{{ monthly_duration }} {{ monthly_unit }}' AND CURRENT_DATE AND (c.is_beginning_of_month = TRUE))
                        {%- endif -%}
                    {% endif -%}
                    ...
                    (c.{{ sdts_alias }} BETWEEN CURRENT_DATE - INTERVAL '{{ yearly_duration }} {{ yearly_unit }}' AND CURRENT_DATE AND (c.is_beginning_of_year = TRUE))
                        {%- endif -%}
                    {% endif %}
                    THEN TRUE
                    ELSE FALSE
