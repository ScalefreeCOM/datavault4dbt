{%- macro trino__nh_link(link_hashkey, foreign_hashkeys, payload, source_models, src_ldts, src_rsrc, disable_hwm, source_is_single_batch, union_strategy, additional_columns) -%}
...
{#  Select all deduplicated records from each source, and filter for records that are newer
    ...
    src_new_{{ source_number }} AS (
        ...
        SELECT
            ...
            {% for col in source_model['payload'] -%}
            {{ col }},
            {% endfor -%}
