{%- macro trino__ref_hub(ref_keys, src_ldts, src_rsrc, source_models, additional_columns) -%}
...
earliest_hk_over_all_sources_prep AS (
    SELECT
        lcte.*,
        ROW_NUMBER() OVER (PARTITION BY {{ datavault4dbt.concat_ws(ref_keys) }} ORDER BY {{ src_ldts
        }}) as rn
