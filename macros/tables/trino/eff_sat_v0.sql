{%- macro trino__eff_sat_v0(source_model, tracked_hashkey, src_ldts, src_rsrc, is_active_alias, source_is_single_batch, disable_hwm, additional_columns) -%}
...
{#
    ...
    Trino does not support QUALIFY, so we use a subquery with ROW_NUMBER instead.
#}
{%- if is_incremental() %}
current_status AS (
