        ...
        CASE 
            ...
            {% if 'daily' in log_logic.keys() %}
                ...
                {%- else %}                            
                  ...
                  (DATE_TRUNC('DAY', c.sdts::DATE) BETWEEN CURRENT_DATE() - INTERVAL '{{ daily_duration }} {{ daily_unit }}' AND CURRENT_DATE())
                {%- endif -%}   
            {%- endif %}
            ...
            {%- if 'weekly' in log_logic.keys() %} OR 
              ...
              ((DATE_TRUNC('DAY', c.sdts::DATE) BETWEEN CURRENT_DATE() - INTERVAL '{{ weekly_duration }} {{ weekly_unit }}' AND CURRENT_DATE()) AND (c.is_weekly = TRUE))
                {%- endif -%}
            {% endif -%}
            ...
            {%- if 'monthly' in log_logic.keys() %} OR
              ...
              ((DATE_TRUNC('DAY', c.sdts::DATE) BETWEEN CURRENT_DATE() - INTERVAL '{{ monthly_duration }} {{ monthly_unit }}' AND CURRENT_DATE()) AND (c.is_monthly = TRUE))
                {%- endif -%}
            {% endif -%}
            ...
            {%- if 'yearly' in log_logic.keys() %} OR 
              ...
              ((DATE_TRUNC('DAY', c.sdts::DATE) BETWEEN CURRENT_DATE() - INTERVAL '{{ yearly_duration }} {{ yearly_unit }}' AND CURRENT_DATE()) AND (c.is_yearly = TRUE))
                {%- endif -%}
            {% endif %}
            THEN TRUE
            ELSE FALSE
