{%- macro sqlserver__sat_v0(parent_hashkey, src_hashdiff, src_payload, src_ldts, src_rsrc, source_model) -%}
...
latest_entries_in_sat AS (
    ...
    SELECT
        {{ parent_hashkey }},
        {{ ns.hdiff_alias }},
        ROW_NUMBER() OVER(PARTITION BY {{ parent_hashkey|lower }} ORDER BY {{ src_ldts }} DESC) latest_rn
    FROM 
        {{ this }}
...
prep_deduplicated_numbered_source AS (
    ...
    {{ ns.hdiff_alias }},
    {{ datavault4dbt.print_list(source_cols) }},
    LAG({{ ns.hdiff_alias }}) OVER(PARTITION BY {{ parent_hashkey|lower }} ORDER BY {{ src_ldts }}) prev_hdiff
    FROM source_data
),
...
records_to_insert AS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE {{ datavault4dbt.multikey(parent_hashkey, prefix=['latest_entries_in_sat', 'deduplicated_numbered_source'], condition='=') }}
            ...
            and latest_entries_in_sat.latest_rn = 1
        )
    {%- endif %}
