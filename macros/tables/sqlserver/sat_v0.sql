{%- macro sqlserver__sat_v0(parent_hashkey, src_hashdiff, src_payload, src_ldts, src_rsrc, source_model, disable_hwm, source_is_single_batch, additional_columns) -%}
...
{%- if has_hashdiff %}{% set src_hashdiff = datavault4dbt.escape_column_names(src_hashdiff) %}{%- endif %}
...
deduplicated_numbered_source_prep AS (
    ...
    {%- endif %}
    {{ datavault4dbt.print_list(source_cols) }}
    , LAG({{ dedup_column }}) OVER(PARTITION BY {{ parent_hashkey }} ORDER BY {{ src_ldts }}) as prev_dedup
    FROM source_data
...
deduplicated_numbered_source AS (
    ...
    WHERE 1=1
        AND {{ dedup_column }} <> prev_dedup OR prev_dedup IS NULL
