{%- macro sqlserver__hub(hashkey, business_keys, src_ldts, src_rsrc, source_models, disable_hwm) -%}
...
earliest_hk_over_all_sources AS (
    ...
    (
        SELECT
            lcte.*,
            ROW_NUMBER() OVER (PARTITION BY {{ hashkey }} ORDER BY {{ src_ldts }}) rn
...
records_to_insert AS (
   ...
   SELECT
    ...
    {%- if is_incremental() %}
        left join distinct_target_hashkeys dest on src.{{ hashkey }} = dest.{{ hashkey }}
    WHERE 
        dest.{{ hashkey }} is null
    {% endif %}
