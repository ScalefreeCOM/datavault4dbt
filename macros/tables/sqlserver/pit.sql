{%- macro sqlserver__pit(tracked_entity, hashkey, sat_names, ldts, ledts, sdts, snapshot_relation, dimension_key, refer_to_ghost_records, snapshot_trigger_column=none, custom_rsrc=none, pit_type=none) -%}
    ...
    FROM
        ...
        LEFT JOIN (
            SELECT
                ...
                {{ ldts }},
                COALESCE(
                    DATEADD(ns, -100, (LEAD{{ ldts }})) OVER (PARTITION BY {{ hashkey }} ORDER BY {{ ldts }}),
