{%- macro sqlserver__pit(tracked_entity, hashkey, sat_names, ldts, ledts, sdts, snapshot_relation, dimension_key,snapshot_trigger_column=none, custom_rsrc=none, pit_type=none) -%}
...
pit_records AS (
    ...
    FROM
        ...
        FULL OUTER JOIN -- Test with CROSS JOIN moving ON contitions to WHERE clause. Might work faster.
        ...
        LEFT JOIN (
            SELECT
                {{ hashkey }},
                {{ ldts }},
                COALESCE(LEAD({{ ldts }} - INTERVAL '1 MICROSECOND' /*This syntax will not work in SQL Server. Use function DATEADD*/) OVER (PARTITION BY {{ hashkey }} ORDER BY {{ ldts }}),{{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }}) AS {{ ledts }}
            FROM {{ ref(satellite) }}
        ) {{ satellite }}
        {% endif %}
            ON
    ...
    {%- endif %}
    {%- if is_incremental() %} -- I would use this incremental flter condition as earlier as possible
        AND te.{{ hashkey }} --NOT IN (SELECT * FROM existing_dimension_keys)
        NOT EXISTS (
            SELECT 1
