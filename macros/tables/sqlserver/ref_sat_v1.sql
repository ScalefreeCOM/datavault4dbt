{%- macro sqlserver__ref_sat_v1(ref_sat_v0, ref_keys, hashdiff, src_ldts, src_rsrc, ledts_alias, add_is_current_flag) -%}
...
end_dated_source AS (
    ...
    SELECT
        ...
        {{ src_rsrc }},
        {{ src_ldts }},
        COALESCE(LEAD(DATEADD(MILLISECOND,-1,{{ src_ldts }})) OVER (PARTITION BY {%- for ref_key in ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }}),{{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }}) as {{ ledts_alias }},
