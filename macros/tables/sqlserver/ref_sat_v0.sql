{%- macro sqlserver__ref_sat_v0(parent_ref_keys, src_hashdiff, src_payload, src_ldts, src_rsrc, source_model, disable_hwm, source_is_single_batch, additional_columns) -%}
...
numbered_source AS (
    ...
    {%- endif %}
    {{ datavault4dbt.print_list(source_cols) }}
    , LAG({{ dedup_column }}) OVER(PARTITION BY {%- for ref_key in parent_ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }}) prev_dedup
    FROM source_data
),

{#
    Deduplicate source by comparing each dedup column to the value of the previous record.
...
deduplicated_numbered_source AS (
    ...
    WHERE {{ dedup_column }} <> prev_dedup OR prev_dedup IS NULL
