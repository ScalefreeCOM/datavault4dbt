{%- macro sqlserver__ref_sat_v0(parent_ref_keys, src_hashdiff, src_payload, src_ldts, src_rsrc, source_model) -%}
...
latest_entries_in_sat AS (
    ...
    (
        SELECT
            ...
            {% endfor %}
            {{ ns.hdiff_alias }},
            ROW_NUMBER() OVER(PARTITION BY {%- for ref_key in parent_ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }} DESC) LatestRowNum  
        FROM 
            {{ this }}
    ) l
    where l.LatestRowNum = 1
),
{%- endif %}
...
#}
prep_deduplicated_numbered_source AS (
    ...
    {{ ns.hdiff_alias }},
    {{ datavault4dbt.print_list(source_cols) }},
    LAG({{ ns.hdiff_alias }}) OVER (PARTITION BY {%- for ref_key in parent_ref_keys %} {{ref_key}} {%- if not loop.last %}, {% endif %}{% endfor %} ORDER BY {{ src_ldts }}) prev_hdiff
    FROM source_data
),
