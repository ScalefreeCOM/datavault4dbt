{%- macro sqlserver__link(link_hashkey, foreign_hashkeys, source_models, src_ldts, src_rsrc, disable_hwm) -%}
...
{{ log('final_columns_to_select: '~final_columns_to_select, false) }}
...
{#  Select all deduplicated records from each source, and filter for records that are newer
    ...
    {{ log('rsrc_static defined: ' ~ ns.has_rsrc_static_defined , true) }}
    {{ log('source_included_before: ' ~ ns.source_included_before[source_model|string] , true) }}
    ...
    {{ log('link_hk: '~link_hk, false) }}


    src_new_{{ source_number }} AS (

        SELECT
            {{ source_model['link_hk'] }} AS {{ link_hashkey }},
...
earliest_hk_over_all_sources AS (
    ...
    (
        SELECT
            lcte.*,
            ROW_NUMBER() OVER (PARTITION BY {{ link_hashkey }} ORDER BY {{ src_ldts }}) rn
...
records_to_insert AS (
    ...
    SELECT
        {{ datavault4dbt.print_list(final_columns_to_select,4,'src') }}
    FROM {{ ns.last_cte }} src

    {%- if is_incremental() %}
        left join distinct_target_hashkeys dest on src.{{ link_hashkey }} = dest.{{ link_hashkey }}
    WHERE 
        dest.{{ link_hashkey }} is null
    {% endif %}
)
