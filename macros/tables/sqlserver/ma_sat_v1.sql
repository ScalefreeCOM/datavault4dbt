{%- macro sqlserver__ma_sat_v1(sat_v0, hashkey, hashdiff, ma_attribute, src_ldts, src_rsrc, ledts_alias, add_is_current_flag) -%}
...
end_dated_loads AS (
    ...
    SELECT
        {{ hashkey }},
        {{ src_ldts }},
        COALESCE(LEAD(DATEADD(MILLISECOND,-1,{{ src_ldts }})) OVER (PARTITION BY {{ hashkey }} ORDER BY {{ src_ldts }}),{{ datavault4dbt.string_to_timestamp( timestamp_format , end_of_all_times) }}) as {{ ledts_alias }}
