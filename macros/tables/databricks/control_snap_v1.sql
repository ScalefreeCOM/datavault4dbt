{%- macro databricks__control_snap_v1(control_snap_v0, log_logic, sdts_alias) -%}
        ...
        {%- else %}
            {% for logic in ns.log_logic_list -%}
                ...
                CASE
                    ...
                    ((c.{{ sdts_alias }} BETWEEN TRY_SUBTRACT(CURRENT_TIMESTAMP(), INTERVAL '{{ weekly_duration }}' {{ weekly_unit }}) AND CURRENT_TIMESTAMP()) AND (c.is_beginning_of_week = TRUE))
                        {%- endif -%}
                    {% endif -%}
                    ...
                    ((c.{{ sdts_alias }} BETWEEN TRY_SUBTRACT(CURRENT_TIMESTAMP(), INTERVAL '{{ monthly_duration }}' {{ monthly_unit }}) AND CURRENT_TIMESTAMP()) AND (c.is_beginning_of_month = TRUE))
                        {%- endif -%}
                    {% endif -%}
                    ...
                    ((c.{{ sdts_alias }} BETWEEN TRY_SUBTRACT(CURRENT_TIMESTAMP(), INTERVAL '{{ yearly_duration }}' {{ yearly_unit }}) AND CURRENT_TIMESTAMP()) AND (c.is_beginning_of_year = TRUE))
                        {%- endif -%}
                    {% endif %}
                    THEN TRUE
                    ELSE FALSE
