{%- macro multikey(columns, prefix=none, condition=none, operator='AND') -%}
    ...
    {{- adapter.dispatch('multikey', 'dbtvault_scalefree')(columns=columns, prefix=prefix, condition=condition, operator=operator) -}}
...
{%- macro default__multikey(columns, prefix=none, condition=none, operator='AND') -%}
    ...
    {%- if condition in ['<>', '!=', '='] -%}
        {%- for col in columns -%}
            {%- if prefix -%}
                {{- dbtvault_scalefree.prefix([col], prefix[0], alias_target='target') }} {{ condition }} {{ dbtvault_scalefree.prefix([col], prefix[1]) -}}
            {%- endif %}
