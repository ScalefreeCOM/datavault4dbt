...
hashed_columns AS (
    ...
    SELECT
      ...
      {{ datavault4dbt.alias_all(columns=datavault4dbt.escape_column_names(final_columns_to_select), prefix='main_hashkey_generation') }},                            {# Everything from last_cte before hashed_columns.#}
...
unknown_values AS (
    ...
    {# Additionally generating Ghost Records for Derived Columns  #}
      {%- for column_name, properties in derived_columns_with_datatypes_DICT.items() %}
        ,{{ datavault4dbt.ghost_record_per_datatype(column_name=datavault4dbt.escape_column_names(column_name), datatype=properties.datatype, col_size=properties.col_size, numeric_precision=properties.get('numeric_precision'), numeric_scale=properties.get('numeric_scale'), ghost_record_type='unknown') }}
      {%- endfor -%}
...
error_values AS (
    ...
    {# Additionally generating Ghost Records for Derived Columns #}
      {%- for column_name, properties in derived_columns_with_datatypes_DICT.items() %}
        ,{{ datavault4dbt.ghost_record_per_datatype(column_name=datavault4dbt.escape_column_names(column_name), datatype=properties.datatype, col_size=properties.col_size, numeric_precision=properties.get('numeric_precision'), numeric_scale=properties.get('numeric_scale'), ghost_record_type='error') }}
      {%- endfor -%}
