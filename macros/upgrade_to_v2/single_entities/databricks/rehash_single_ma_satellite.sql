...
{% macro databricks__rehash_single_ma_satellite(ma_satellite, hashkey, hashdiff, ma_keys, payload, parent_entity, business_keys, overwrite_hash_values=false, output_logs=true, drop_old_values=true) %}

    {% set ma_satellite_relation = ref(ma_satellite) %}
    {% set parent_relation = ref(parent_entity) %}

    {# Check Relation Type #}
    {% set target_relation = load_relation(ma_satellite_relation) %}
    ...
    {{ log('Enabling Delta Column Mapping on ' ~ ma_satellite_relation ~ '...', output_logs) }}
    {% set enable_mapping_sql %}
        ALTER TABLE {{ ma_satellite_relation }} SET TBLPROPERTIES (
    ...
    {% set potential_stuck_cols = [
        new_hashkey_name, 
        new_hashdiff_name, 
        hashkey + '_deprecated', 
        hashdiff + '_deprecated'
    ] %}

    {% for col_name in potential_stuck_cols %}
        {% if col_name in existing_col_names %}
            {{ log('Dropping stuck column ' ~ col_name, true) }}
            {% do run_query("ALTER TABLE " ~ ma_satellite_relation ~ " DROP COLUMN " ~ col_name) %}
        {% endif %}
    {% endfor %}
    ...
    {% if overwrite_hash_values %}
        ...
        {# Rename Hashkey (Old->Deprecated, New->Old) #}
        {% set r1 = datavault4dbt.get_rename_column_sql(relation=ma_satellite_relation, old_col_name=hashkey, new_col_name=hashkey + '_deprecated') %}
        {% do run_query(r1) %}
        {% set r2 = datavault4dbt.get_rename_column_sql(relation=ma_satellite_relation, old_col_name=new_hashkey_name, new_col_name=hashkey) %}
        {% do run_query(r2) %}

        {# Rename Hashdiff (Old->Deprecated, New->Old) #}
        {% set r3 = datavault4dbt.get_rename_column_sql(relation=ma_satellite_relation, old_col_name=hashdiff, new_col_name=hashdiff + '_deprecated') %}
        {% do run_query(r3) %}
        {% set r4 = datavault4dbt.get_rename_column_sql(relation=ma_satellite_relation, old_col_name=new_hashdiff_name, new_col_name=hashdiff ) %}
        {% do run_query(r4) %}
        ...
        {% if drop_old_values %}
            ...
            {% for col in columns_to_drop %}
                {% set drop_sql %}
                    ALTER TABLE {{ ma_satellite_relation }} DROP COLUMN IF EXISTS {{ col.name }}
                {% endset %}
...
{% macro databricks__ma_satellite_update_statement(ma_satellite_relation, new_hashkey_name, new_hashdiff_name, hashkey, business_key_list, ma_keys, ldts_col, hash_config_dict, parent_relation) %}
    ...
    USING (
        ...
        {# Pass-through for Unknown/Error #}
        SELECT
            sat.{{ hashkey }},
            sat.{{ ldts_col }},
            {{ datavault4dbt.print_list(ma_keys) }},
    ...
    {# Join Condition for Multi-Active Satellite (HK + LDTS + MA_Keys) #}
