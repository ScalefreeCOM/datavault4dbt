...
{% macro exasol__ma_satellite_update_statement(ma_satellite_relation, new_hashkey_name, new_hashdiff_name, hashkey, business_key_list, ma_keys, ldts_col, hash_config_dict, parent_relation) %}
    ...
    {% set ns = namespace(update_on_condition='', parent_already_rehashed=false) %}
    ...
    {% for ma_key in ma_keys %}
        ...
        {% endset %}
        {% set ns.update_on_condition = ns.update_on_condition + where_condition %}
    {% endfor %}

    {# Complete the MERGE statement with the ON condition and UPDATE SET clause #}
    {% set update_sql = update_sql + ns.update_on_condition %}
    ...
    {% set update_sql = update_sql + update_sql_set_clause %}
