...
{% macro sqlserver__rehash_single_hub(hub, hashkey, business_keys, overwrite_hash_values=false, output_logs=true, drop_old_values=true) %}
    ...
    {% if overwrite_hash_values %}
        ...
        {% set overwrite_sql %}
        {{ datavault4dbt.custom_get_rename_column_sql(relation=hub_relation, old_col_name=hashkey, new_col_name=hashkey + '_deprecated') }}
        {{ datavault4dbt.custom_get_rename_column_sql(relation=hub_relation, old_col_name=new_hashkey_name, new_col_name=hashkey) }}
        {% endset %}
...
{% macro sqlserver__hub_update_statement(hub_relation, new_hashkey_name, hashkey, hash_config_dict) %}
