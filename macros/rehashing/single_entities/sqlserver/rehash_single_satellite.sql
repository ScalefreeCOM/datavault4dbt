...
{% macro sqlserver__rehash_single_satellite(satellite, hashkey, hashdiff, payload, parent_entity, business_keys=none, overwrite_hash_values=false, output_logs=true, drop_old_values=true) %}
...
{% macro sqlserver__satellite_update_statement(satellite_relation, new_hashkey_name, new_hashdiff_name, hashkey, ldts_col, hash_config_dict, parent_relation) %}
    ...
    {% for column in all_parent_columns %}
        {% if column.name|lower == hashkey|lower + '_deprecated' %}
            ...
            {% if var('datavault4dbt.show_debug_logs', false) %}{{ log('Parent already hashed, using rehashed value for ' ~ satellite_relation.name, false) }}{% endif %}
        {% endif %}
    {% endfor %}
    ...
    AND nh.{{ hashkey }} = {{ satellite_relation }}.{{ hashkey }}
