...
{% macro sqlserver__rehash_single_nh_satellite(nh_satellite, hashkey, parent_entity, business_keys=none, overwrite_hash_values=false, output_logs=true, drop_old_values=true) %}
...
{% macro sqlserver__nh_satellite_update_statement(nh_satellite_relation, new_hashkey_name, hashkey, ldts_col, parent_relation, hash_config_dict=none) %}
    ...
    {% for column in all_parent_columns %}
        {% if column.name|lower == hashkey|lower + '_deprecated' %}
            ...
            {% if var('datavault4dbt.show_debug_logs', false) %}{{ log('Parent already hashed, using rehashed value for ' ~ nh_satellite_relation.name, false) }}{% endif %}
        {% endif %}
    {% endfor %}
