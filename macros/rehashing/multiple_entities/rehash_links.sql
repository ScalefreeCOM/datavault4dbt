...
{% macro rehash_links(link_yaml, drop_old_values=true) %}
    ...
    {% if link_dict.get('links') is not none %}
        ...
        {% for link in link_dict.get('links') %}
            ...
            {% set additional_hash_input_cols = link.additional_hash_input_cols if link.additional_hash_input_cols is not undefined else [] %}
