{# ============================================================
   ...
   1. expand_target_column_types: the built-in trino__alter_column_type
      ...
      We replace it with CTAS + DROP + RENAME.
...
{% materialization incremental, adapter='trino', supported_languages=['sql'] -%}
  ...
  {{ log('[datavault4dbt] Using Trino incremental override materialization', true) }}
  ...
  {% else %}
    ...
    {{ trino__expand_column_types(tmp_relation, target_relation) }}
