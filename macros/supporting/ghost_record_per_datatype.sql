...
{%- set unknown_value = var('dbtvault_scalefree.unknown_value_ghost_record', '(unknown)') -%}
{%- set error_value = var('dbtvault_scalefree.error_value_ghost_record', '(error)') -%}

{%- if ghost_record_type == 'unknown' -%}

        {%- if datatype == 'TIMESTAMP' %} {{ dbtvault_scalefree.string_to_timestamp( timestamp_format , beginning_of_all_times) }} as {{ column_name }}
        {%- elif datatype == 'STRING' %} '{{unknown_value_ghost_record}}' as {{ column_name }}
...
{%- elif ghost_record_type == 'error' -%}
        ...
        {%- elif datatype == 'STRING' %} '{{error_value_ghost_record}}' as {{ column_name }}
...
{%- if ghost_record_type == 'unknown' -%}
        ...
        {%- if datatype == 'TIMESTAMP' %} {{ dbtvault_scalefree.string_to_timestamp( timestamp_format , beginning_of_all_times) }} as "{{ column_name }}"
        {%- elif datatype.upper().startswith('VARCHAR') -%} '{{unknown_value_ghost_record}}' as "{{ column_name }}"
...
{%- elif ghost_record_type == 'error' -%}
        ...
        {%- if datatype == 'TIMESTAMP' %} {{ dbtvault_scalefree.string_to_timestamp( timestamp_format , end_of_all_times) }} as "{{ column_name }}"
        {%- elif datatype.upper().startswith('VARCHAR') -%} '{{error_value_ghost_record}}' as "{{ column_name }}"
