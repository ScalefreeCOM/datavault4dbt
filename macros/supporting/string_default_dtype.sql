...
{%- macro sqlserver__string_default_dtype(type) %}
...
{%- if global_var is mapping -%}    
    {%- if 'sqlserver' in global_var.keys()|map('lower') -%}
        {% set string_default_dtype = global_var['sqlserver'] %}
    {%- else -%}
        {%- if execute -%}
            {%- do exceptions.warn("Warning: You have set the global variable 'datavault4dbt." ~ type ~ "_default_dtype' to a dictionary, but have not included the adapter you use (sqlserver) as a key. Applying the default value.") -%}
        {% endif %}
        {%- set string_default_dtype = "VARCHAR(255)" -%}
    {% endif %}
{%- elif global_var is not mapping and datavault4dbt.is_something(global_var) -%}
