...
{%- macro default__first_day_of_week() %}
    ...
    {%- if global_var is mapping -%}
        ...
        {%- else -%}
            {%- if execute -%}
                {%- do exceptions.warn("Warning: Adapter: '"~ target.type ~"' not found in 'datavault4dbt.first_day_of_week' dictionary. Applying default (1).") -%}
            {%- endif -%}
        {%- endif -%}
    ...
    {%- else -%}
        {%- set first_day = 1 -%}
        {%- do log("Debug: Global variable 'datavault4dbt.first_day_of_week' is missing or invalid. Falling back to default (1).") -%}
    {%- endif -%}
